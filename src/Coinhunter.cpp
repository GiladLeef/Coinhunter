...
Coinhunter::Coinhunter(const std::string& inputFile, int compMode, int searchMode, int coinType, bool useGpu,
...
Coinhunter::Coinhunter(const std::vector<unsigned char>& hashORxpoint, int compMode, int searchMode, int coinType,
...
void Coinhunter::InitGenratorTable()
{
...
void Coinhunter::output(std::string addr, std::string pAddr, std::string pAddrHex, std::string pubKey)
{
...
bool Coinhunter::checkPrivKey(std::string addr, Int& key, int32_t incr, bool mode)
{
	Int k(&key), k2(&key);
	k.Add((uint64_t)incr);
...
bool Coinhunter::checkPrivKeyETH(std::string addr, Int& key, int32_t incr)
{
	Int k(&key), k2(&key);
	k.Add((uint64_t)incr);
	k2.Add((uint64_t)incr);
...
bool Coinhunter::checkPrivKeyX(Int& key, int32_t incr, bool mode)
{
	Int k(&key);
	k.Add((uint64_t)incr);
	Point p = secp->ComputePublicKey(&k);
...
void Coinhunter::checkMultiAddresses(bool compressed, Int key, int i, Point p1)
{
	unsigned char h0[20];
...
void Coinhunter::checkMultiAddressesETH(Int key, int i, Point p1)
{
	unsigned char h0[20];
...
void Coinhunter::checkSingleAddress(bool compressed, Int key, int i, Point p1)
{
	unsigned char h0[20];
...
void Coinhunter::checkSingleAddressETH(Int key, int i, Point p1)
{
	unsigned char h0[20];
...
void Coinhunter::checkMultiXPoints(bool compressed, Int key, int i, Point p1)
{
	unsigned char h0[32];
...
void Coinhunter::checkSingleXPoint(bool compressed, Int key, int i, Point p1)
{
	unsigned char h0[32];
...
void Coinhunter::checkMultiAddressesSSE(bool compressed, Int key, int i, Point p1, Point p2, Point p3, Point p4)
{
	unsigned char h0[20];
	unsigned char h1[20];
	unsigned char h2[20];
...
void Coinhunter::checkSingleAddressesSSE(bool compressed, Int key, int i, Point p1, Point p2, Point p3, Point p4)
{
	unsigned char h0[20];
	unsigned char h1[20];
	unsigned char h2[20];
...
void Coinhunter::getCPUStartingKey(Int & tRangeStart, Int & tRangeEnd, Int & key, Point & startP)
{
	if (rKey <= 0) {
		key.Set(&tRangeStart);
	}
...
void Coinhunter::FindKeyCPU(TH_PARAM * ph)
{
...
void Coinhunter::getGPUStartingKeys(Int & tRangeStart, Int & tRangeEnd, int groupSize, int nbThread, Int * keys, Point * p)
{
...
void Coinhunter::FindKeyGPU(TH_PARAM * ph)
{
...
#else
	ph->hasStarted = true;
	printf("This version of Coinhunter was compiled to support CPU only, Please recompile with CUDA support or open a ticket on github.\n");
#endif
...
bool Coinhunter::isAlive(TH_PARAM * p)
{
...
bool Coinhunter::hasStarted(TH_PARAM * p)
{
...
uint64_t Coinhunter::getGPUCount()
{
...
uint64_t Coinhunter::getCPUCount()
{
...
void Coinhunter::rKeyRequest(TH_PARAM * p) {
...
void Coinhunter::SetupRanges(uint32_t totalThreads)
{
	Int threads;
...
void Coinhunter::Search(int nbThread, std::vector<int> gpuId, std::vector<int> gridSize, bool& should_exit)
{
...
std::string Coinhunter::GetHex(std::vector<unsigned char> &buffer)
{
	std::string ret;
...
int Coinhunter::CheckBloomBinary(const uint8_t * _xx, uint32_t K_LENGTH)
{
...
bool Coinhunter::MatchHash(uint32_t * _h)
{
...
bool Coinhunter::MatchXPoint(uint32_t * _h)
{
	if (_h[0] == xpoint[0] &&
		_h[1] == xpoint[1] &&
		_h[2] == xpoint[2] &&
...
std::string Coinhunter::formatThousands(uint64_t x)
{
	char buf[32] = "";
...
char* Coinhunter::toTimeStr(int sec, char* timeStr)
{
	int h, m, s;
	h = (sec / 3600);
	m = (sec - (3600 * h)) / 60;
